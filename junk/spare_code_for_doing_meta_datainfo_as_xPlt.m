...
function di = update_metadata_packedDims(di,packed_vars,packed_name)
    ...
    % Pull out MDict object storing metadata. This object holds metadata
    ...
    % Create the new metadata MDict object
    sz = size(did);
...
% Store metadata info
    ...
    % Create dummy MDict object full of ones in order to store axis info.
    di = MDict;
